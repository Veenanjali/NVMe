...
def run_fio_test():
    command = [
        'fio', 
        '--name=sequential-read-test',  # Name of the test
        '--ioengine=libaio', 
        '--rw=read',  # Change to read for sequential reads
        '--bs=4k', 
        '--numjobs=1', 
        '--runtime=10', 
        '--iodepth=32', 
    ...
    print("Running sequential read tests on NVMe device")
    ...
    total_ios = output['jobs'][0]['read']['total_ios']  # Total I/O operations for read
    iops = output['jobs'][0]['read']['iops']  # IOPS for read
