...
def run_fio_test():
    command = [
        'fio', 
        '--name=sequential-write-test',  # Change the job name to indicate it's a write test
        '--ioengine=libaio', 
        '--rw=write',  # Change to write for sequential writes
        '--bs=4k', 
        '--numjobs=1', 
        '--runtime=10', 
        '--iodepth=32', 
    ...
    print("Running sequential write tests on NVMe device")
